...
import pickle
from datasets.roi_data_layer.imdb import imdb
from datasets.roi_data_layer.imdb import ROOT_DIR
from datasets.roi_data_layer import kitti_utils
...
class kitti(imdb):
    ...
    def point_cloud_path_at(self, i):
        '''
        Return the absolute path to image i in the image sequence.
        '''
        return self.point_cloud_path_from_index(self._image_index[i])
    ...
    def point_cloud_path_from_index(self, index):
        '''
        Construct an image path from the image's "index" identifier.
        '''
        if self._image_set == 'test':
